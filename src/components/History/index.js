...
class History extends Component {
  state = {userInput: '', userDetailsList: initialHistoryList, isTrue: false}

  fillFunction = id => {
    const {userDetailsList} = this.state
    const filterData = userDetailsList.filter(each => each.id !== id)
    if (filterData.length === 0) {
      this.setState({userDetailsList: filterData, isTrue: true})
    } else {
